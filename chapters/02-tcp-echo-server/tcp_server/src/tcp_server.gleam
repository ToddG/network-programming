...
import logging
import socket_options.{type TcpOption, Binary, Mode, ActiveMode, Active, Debug}

@external(erlang, "tcp_ffi", "listen")
pub fn listen( port: Int, options: List(TcpOption), ) -> Result(ListenSocket, SocketReason)
...
pub fn main() {
  ...
  let assert Ok(socket) = listen(4000, [Debug(True), Mode(Binary), ActiveMode(Active)])
